...
$(document).delegate('#deleteAll','click',function() {
    ...
    if(theArray.length<1){
        ...
        $("#deleteAllModal").find(".modal-body").append('<h5>您确定要删除选中的<strong><span class="text-danger">'+theArray.length+'</span></strong>条记录吗？</h5>');
...
/**
 * 删除界面不可见时清空modal-body内容
 */
$('#deleteAllModal').on('hidden.bs.modal', function () {
    $("#deleteAllModal").find(".modal-body").empty();
});
/**
 * 单击修改按钮
 */
